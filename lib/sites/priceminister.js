...
class PriceMinisterSite {
  ...
  findPriceOnPage($) {
    ...
    // if we've not found the price, try to use the itemprop meta tags
    if (!price) {
      logger.log('unable to find price using normal methods, trying meta tags...');
      price = $('*[itemprop="lowPrice"]').attr('content');
    }
