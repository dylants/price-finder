...
var siteUtils = require("../site-utils"),
    debug = require("debug")("price-finder:amazon");
...
function AmazonSite(uri) {
    ...
    this.findPriceOnPage = function($) {
        ...
        // were we successful?
        if (!price) {
            debug($("body").text().trim());
            debug("price not found on amazon page, uri: " + this._uri);
            return -1;
        }
    ...
    this.findCategoryOnPage = function($) {
        ...
        if (!category || category.length < 1) {
            debug("category not found on amazon page, uri: " + this._uri);
            return null;
        }
    ...
    this.findNameOnPage = function($, category) {
        ...
        if (!name) {
            debug("name not found on amazon page, uri: " + this._uri);
            return null;
        }
