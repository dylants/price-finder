...
function AmazonSite(uri) {
    ...
    this.findPriceOnPage = function($) {
        ...
        // were we successful?
        if (!price) {
            logger.log($("body").text().trim());
            logger.error("price not found on amazon page, uri: " + uri);
            return -1;
        }
    ...
    this.findCategoryOnPage = function($) {
        ...
        if (!category || category.length < 1) {
            logger.error("category not found on amazon page, uri: " + uri);
            return null;
        }
    ...
    this.findNameOnPage = function($, category) {
        ...
        if (!name) {
            logger.error("name not found on amazon page, uri: " + uri);
            return null;
        }
