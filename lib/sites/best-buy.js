...
var siteUtils = require("../site-utils"),
    debug = require("debug")("price-finder:best-buy");
...
function BestBuySite(uri) {
    ...
    this.findPriceOnPage = function($) {
        ...
        if (!price) {
            debug("price was not found on best buy page, uri: " + this._uri);
            return -1;
        }
    ...
    this.findCategoryOnPage = function($) {
        ...
        if (!category || category.length < 1) {
            debug("category not found on best buy page, uri: " + this._uri);
            return null;
        }

        debug("raw category found: " + category);
        ...
        if (category === "Movies & Music") {
            ...
            debug("raw parentCategory: " + parentCategory);
    ...
    this.findNameOnPage = function($, category) {
        ...
        if (!name || name.length < 1) {
            debug("name not found on best buy page, uri: " + this._uri);
            return null;
        }
        name = name.trim();
