...
exports.processPrice = function processPrice(priceStringInput) {
  ...
  if (_.includes(priceString, DOLLAR_PREFIX)) {
    ...
    let tempPriceString = priceString;
    tempPriceString = tempPriceString.substring(2, tempPriceString.length);
    tempPriceString = tempPriceString.replace(',', '');
    price = accounting.unformat(tempPriceString);
  } else {
    // unknown price type!!
