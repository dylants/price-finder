...
var fs = require("fs"),
    debug = require("debug")("price-finder:site-manager");
...
function loadAllSites() {
    ...
    fs.readdirSync(__dirname + "/sites").forEach(function(site) {
        ...
        debug("loading site module: " + site);
...
function SiteManager() {
    debug("initializing SiteManager");
    ...
    this.loadSite = function(uri) {
        ...
        debug("attempting to find site for uri: " + uri);

        // loop over all our sites
        for (i = 0; i < this.sites.length; i++) {
            // if one matches...
            if (this.sites[i].isSite(uri)) {
                debug("match found, using site: " + this.sites[i].name);
